...
# ---------------------------------------------------
#   1. modifyLoinInfo : 安装Firewalld并添加终端登录提示信息
# ---------------------------------------------------
modifyLoinInfo() {
    echo -e "\n开始执行,请耐心等待..."
    if ! [ -x "$(command -v firewalld)" ]; then
        echo -e "\n正在安装防火墙firewalld,请耐心等待......"
        yum install firewalld -y >/dev/null 2>&1 || error_exit "yum安装firewalld失败,退出！！"
...
EOF
    ...
    # 配置防火墙并启用服务
    echo -e "\n为shadowsocks配置防火墙,配置文件位于$SS_FIREWALLD_CONF..."
    if [ ! -f "$SS_FIREWALLD_CONF" ]; then
        touch $SS_FIREWALLD_CONF
        cat << EOF > $SS_FIREWALLD_CONF
...
EOF
        ...
        mv $SS_FIREWALLD_CONF $SS_FIREWALLD_CONF"_bak"
        touch $SS_FIREWALLD_CONF
        cat << EOF > $SS_FIREWALLD_CONF
        <?xml version="1.0" encoding="utf-8"?>
        <service>
...
EOF
    ...
    # Delete the 8 spaces
    sed -i 's/^[][ ]*//g' $SS_FIREWALLD_CONF
    systemctl enable shadowsocks-libev >/dev/null 2>&1
    echo -e "\n正在重新配置和加载firewalld防火墙..."
    systemctl restart firewalld >/dev/null 2>&1
    firewall-cmd --reload 
...
EOF
    ...
    # 添加防火墙并启动服务
    # 配置防火墙并启用服务
    echo -e "\n为Squid配置防火墙,配置文件位于$SQ_FIREWALLD_CONF"
    if [ ! -f "$SQ_FIREWALLD_CONF" ]; then
        touch $SQ_FIREWALLD_CONF
        cat << EOF > $SQ_FIREWALLD_CONF
        <?xml version="1.0" encoding="utf-8"?>
        <service>
            <short>squid</short>
            ...
            <port protocol="tcp" port="$SQ_PORT"/>
        </service>
EOF
    else
        mv $SQ_FIREWALLD_CONF $SQ_FIREWALLD_CONF"_bak"
        touch $SQ_FIREWALLD_CONF
        cat << EOF > $SQ_FIREWALLD_CONF
        <?xml version="1.0" encoding="utf-8"?>
        <service>
